...
print "Combien y a-t-il de journalistes dans ce array ?  "
puts journalists.length

#2 Combien d'handle contiennent un numéro ?
journalists2 = journalists.select{|num| num =~ /[1234567890]/ }
print "Combien d'handle contiennent un numéro ?  "
puts journalists2.length

#3 Combien d'handle contiennent les 4 lettres du prénom "Aude" à la suite (sans prendre en compte les majuscules) ?
journalists3 = journalists.select{|a| a.include? "aude".downcase }
print "Combien d'handle contiennent les 4 lettres du prénom Aude à la suite ?  "
puts journalists3.length

#4 Combien commencent par une majuscule (première lettre juste après le @) ?
journalists4 = journalists.map{|e| e.gsub("@", "")} #attention à ne pas oubier la deuxième partie dans gsub
journalists4 = journalists4.select{|word| word.start_with? /[A-Z]/}
print "Combien commencent par une majuscule (première lettre juste après le @) ?  "
puts journalists4.length

#5 Combien contiennent une majuscule ?
journalists5 = journalists.select{|b| b =~ /[A-Z]/}
print "Combien contiennent une majuscule ?  "
puts journalists5.length

#6 Combien y a-t-il de underscore _ dans tous les pseudos confondus ?
journalists6 = journalists.join(",")
print "Combien y a-t-il de underscore _ dans tous les pseudos confondus ?  "
puts journalists6.count("_")

#7 Trie la liste de handle par ordre alphabétique.
journalists7 = journalists.sort
print "Trie la liste de handle par ordre alphabétique. (verif par la taille)  "
puts journalists7.length

#8 Quels sont les 50 handles les plus courts de ce array ?
journalists8 = journalists.sort_by{|s| s.length}
print "Quels sont les 50 handles les plus courts de ce array ? (verif par la taille)  "
puts journalists7[0..49].length
